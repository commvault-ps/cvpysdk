...
class Organizations:
    ...
    def __str__(self) -> str:
        ...
        for index, organization in enumerate(self._organizations):
            sub_str = f"{index + 1:^5}\t{organization:40}\n"
    ...
    def __getitem__(self, value: str) -> str:
        ...
        try:
            return list(filter(lambda x: x[1]["id"] == value, self.all_organizations.items()))[0][
                0
            ]
        except IndexError:
            ...
            if response.json() and "providers" in response.json():
                ...
                for provider in response.json()["providers"]:
                    ...
                    if self._commcell_object.is_global_scope() and "idpCompanyDetails" in provider:
                        ...
                        self._adv_config[name]["home_commcell"] = self._adv_config[name].get(
                            "home_commcell"
                        )
                        # so home_commcell = None indicates this company is an orphaned proxy
                        continue

                    cloud_service_organizations = (
                        provider.get("organizationCloudServiceDetails", [{}])[0]
                        .get("cloudService", {})
                        .get("redirectUrl")
                    )
                    ...
                    ext_regions = list(
                        {
                            ...
                            for region in provider.get("extendedRegions", [])
                        }
                    )
                    ...
                    self._adv_config[name] = self._adv_config.get(name, {}) | {
                        ...
                        "full_name": [
                            ...
                            for contact in provider.get("primaryContacts", [])
                        ],
    ...
    def _get_fl_parameters(self, fl: list = None) -> str:
        ...
        if fl:
            if all(col in self.valid_columns for col in fl):
                fl_parameters = (
                    f"fl={default_columns},{','.join(self.valid_columns[column] for column in fl)}"
                )
            else:
    ...
    def get_organizations_cache(self, hard: bool = False, **kwargs) -> dict:
        ...
        params = [
            fl_parameters,
            fq_parameters,
            "&includeDeletedCompanies=true"
            if kwargs.get("include_deleted_companies", False)
            else "",
            limit_parameters,
            sort_parameters,
            hard_refresh,
            ...
            for provider in response.json()["providers"]:
                ...
                if self._commcell_object.is_global_scope():
                    organization_config.update(
                        {
                            "commcell": provider.get("commcell", {})
                            .get("entityInfo", {})
    ...
    def add(
        ...
        if not (
            ...
            and isinstance(company_alias, str)
        ):
            raise SDKException("Organization", "101")
            ...
            if not isinstance(default_plans, list):
                ...
                for plan in default_plans:
                    if not self._commcell_object.plans.has_plan(plan):
                        raise SDKException(
                            "Organization", "102", f'Plan: "{plan}" does not exist on Commcell'
                        )
                    else:
                        temp_plan = self._commcell_object.plans.get(plan)
                        temp = {"plan": {"planId": int(temp_plan.plan_id)}}
        ...
        request_json = {
            "organizationInfo": {
                ...
                "organization": {
                    ...
                    "shortName": {"domainName": company_alias},
                },
                "organizationProperties": {
                    "enableAutoDiscovery": enable_auto_discover,
                    "primaryDomain": primary_domain,
                    "primaryContacts": [{"fullName": contact_name, "email": email}],
                },
        ...
        if response.json():
            if "response" in response.json():
                ...
                if error_code == 0:
                    ...
                    if service_commcells:
                        self._commcell_object.service_commcells.associations.add(
                            org_object, service_commcells
                        )
                    return self.get(name)

                raise SDKException("Organization", "107", f"Response: {response.json()}")
    ...
    def delete(self, name: str, deactivate: bool = True) -> None:
        ...
        if flag:
            if response.json():
                ...
                if error_code != 0:
                    raise SDKException("Organization", "104", f'Error: "{error_message}"')
            else:
                raise SDKException("Response", "102")
        else:
            response_string = self._update_response_(response.text)
...
class Organization:
    ...
    def tfa(self) -> TwoFactorAuthentication:
        ...
        if self._tfa_obj is None:
            self._tfa_obj = TwoFactorAuthentication(
                self._commcell_object, organization_id=self._organization_id
            )
    ...
    def set_auto_laptop_owners(
        self, client_assign_type: int, client_assign_value: str = None
    ) -> None:
        ...
        if client_assign_type == 3:
            self._update_properties_json(
                {
                    "autoClientOwnerAssignmentType": client_assign_type,
    ...
    def __repr__(self) -> str:
        """Returns the string representation of an instance of this class."""
        return f'Organization class instance for Organization: "{self.organization_name}"'
    ...
    def _get_properties(self, **params: dict) -> dict:
        ...
        if flag:
            if response.json() and "organizationInfo" in response.json():
                ...
                self._organization_name = self._organization_info["organization"].get(
                    "connectName"
                )
                return self._organization_info
            else:
    ...
    def get_security_associations(self) -> dict:
        ...
        if flag:
            if response.json():
                ...
                security_list = (
                    response.get("securityAssociations")[0]
                    .get("securityAssociations")
                    .get("associations", [])
                )
                for list_item in security_list:
                    name = (
                        list_item.get("userOrGroup")[0].get("userGroupName")
                        or list_item.get("userOrGroup")[0].get("userName")
                        or list_item.get("userOrGroup")[0].get("providerDomainName")
                        + "\\"
                    ...
                    if list_item.get("properties").get("role"):
                        ...
                        for sub_list_item in (
                            list_item.get("properties")
                            .get("categoryPermission")
                            .get("categoriesPermissionList")
                        ):
    ...
    def update_security_associations(
        ...
        req_json = {
            "entityAssociated": {"entity": [{"providerId": int(self.organization_id)}]},
        ...
        flag, response = self._cvpysdk_object.make_request(
            "POST", self._services["SECURITY_ASSOCIATION"], req_json
        )
        ...
        if flag:
            if response.json():
                if "errorMessage" in response.json():
                    ...
                    error_code = response.json()["errorCode"]
                    if error_code != 0:
                        raise SDKException("Organization", "110", f"Error: {error_message}")
            else:
                raise SDKException("Response", "102")
        else:
            response_string = self._update_response_(response.text)
    ...
    def is_download_software_from_internet_enabled(self) -> bool:
        ...
        """
        return bool(self.organization_properties.get("clientGroupForceClientSideDownload"))
    ...
    def shared_laptop(self) -> bool:
        ...
        """
        return not self.organization_info["organizationProperties"].get(
    ...
    def contacts(self, user_names: list[str]) -> None:
        ...
        for user_name in user_names:
            user_list.append({"user": {"userName": user_name}})

        req_json = {"primaryContactsOperationType": "OVERWRITE", "primaryContacts": user_list}
    ...
    def operators(self) -> list[tuple[str, str]]:
        ...
        return sorted(
            [
                (
                    op.get("user", {}).get("userName")
                    or op.get("userGroup", {}).get("userGroupName"),
    ...
    def _lookup_operator(self, user_or_group: str | User | UserGroup, role: str | Role) -> dict:
        ...
        for operator in self.organization_properties.get("operators", []):
            ...
            this_user_or_group = operator.get("user", {}).get("userName") or operator.get(
                "userGroup", {}
            ).get("userGroupName")
    ...
    def _prepare_operator_dict(
        ...
        if isinstance(user_or_group, str):
            ...
            if self._commcell_object.users.has_user(user_or_group):
                ...
                raise SDKException(
                    "Organization", "110", f"Invalid User or User Group name: {user_or_group}"
                )
        ...
        if isinstance(user_or_group, User):
            ...
            raise SDKException(
                "Organization", "110", f"Invalid User or User Group object: {user_or_group}"
            )
        return op_dict
        ...
        return {
            "organizationInfo": {
                ...
                "organizationProperties": {
                    ...
                    "operators": [
                        self._prepare_operator_dict(ug, role) for ug, role in operator_list
                    ],
                },
            }
        }
    ...
    def manage_operators(
        ...
        """
        payload = self._prepare_operators_payload(operator_list)
        payload["organizationInfo"]["organizationProperties"]["operatorsOperationType"] = (
            "OVERWRITE"
        )
        self._commcell_object.wrap_request(
            "PUT",
    ...
    def default_plan(self, value: dict | str) -> None:
        ...
        if not value:
            ...
            temp = [
                {
                    "plan": {"planId": int(temp_plan.plan_id), "planName": temp_plan.plan_name},
    ...
    def update_email_settings(self, email_settings: dict) -> None:
        ...
        if name is None or email is None:
            raise SDKException(
                "Organization", 114, "No configurable settings were set in the input"
            )
        req_json = {"senderSmtp": email, "senderName": name}
        self._update_properties_json(req_json)
        self._update_properties()
    ...
    def file_exceptions(self) -> dict[str, list[str]]:
        ...
        file_exceptions = {}
        for file_filter in self.organization_properties["globalFiltersInfo"][
            "globalFiltersInfoList"
        ]:
            file_exceptions[os_type_map[file_filter["operatingSystemType"]]] = file_filter[
                "globalFilters"
            ].get("filters", [])
                ...
                for os, filters in exceptions.items():
                    temp = {
                        "globalFilters": {
                            ...
                            else self.file_exceptions[os] + filters,
                        },
                        "operatingSystemType": "WINDOWS_GLOBAL_FILTER"
                        if os == "Windows"
                        else "UNIX_GLOBAL_FILTER",
                    }

                    filter_list.append(temp)

                req_json = {"globalFiltersInfo": {"globalFiltersInfoList": filter_list}}
    ...
    def is_company_privacy_enabled(self) -> bool:
        ...
        """
        return (self.organization_properties.get("privacy") or {}).get("enableDataSecurity")
    ...
    def tenant_operator(self) -> dict:
        ...
        """
        tenant_operators = self.organization_info.get("organizationProperties", {}).get(
            "operators", []
        )
        user_list = []
        usergroup_list = []
    ...
    def add_user_groups_as_operator(self, user_group_list: list, request_type: str) -> None:
        ...
        """
        update_operator_request_type = {"NONE": 0, "OVERWRITE": 1, "UPDATE": 2, "DELETE": 3}
    ...
    def operator_role(self, role_name: str) -> None:
        ...
        request_role = {"operatorRole": {"roleId": role_object.role_id, "roleName": role_name}}
    ...
    def deactivate(
        self, disable_backup: bool = True, disable_restore: bool = True, disable_login: bool = True
    ) -> None:
        """
    ...
    def set_country(self, country_name: str) -> None:
        ...
        """
        request_json = {"companyId": int(self.organization_id), "countryName": country_name}
        ...
        if flag:
            if response.json():
                ...
                if error_code != 0:
                    ...
                    raise SDKException("Organization", "102", f'Error: "{error_message}"')
            else:
                raise SDKException("Response", "102")
        else:
            response_string = self._update_response_(response.text)
        ...
        if flag:
            if response and response.json():
                if "response" in response.json():
                    ...
                    resp = response.json()
                    got_ownership_type = (
                        resp.get("organizationInfo", {})
                        .get("organizationProperties", {})
                        .get("autoClientOwnerAssignmentType")
                    )
                    ...
                    if got_ownership_type != want_ownership_type:
                        raise SDKException(
                            "Organization",
                            "102",
                            f"Error: autoClientOwnerAssignmentType value not set correctly. \
    ...
    def add_client_association(self, client_name: str) -> None:
        ...
        request_json = {"entities": [{"clientId": int(client_obj.client_id), "_type_": 3}]}
        flag, response = self._cvpysdk_object.make_request(
    ...
    def retire_offline_laptops(self, retire_days: int, delete_days: int | None = None) -> None:
        ...
        if delete_days != -1 and retire_days > delete_days:
            raise SDKException(
                Organization,
                101,
                "The number of days specified to \
    ...
    def tags(self) -> dict[str, str]:
        ...
        return {tag["name"]: tag.get("value", "") for tag in self._tags}
    ...
    def _prepare_tag_dict(self, tag_name: str, tag_value: str) -> dict[str, str]:
        ...
        return {"name": tag_name, "value": tag_value}
    ...
    def tags(self, tags_dict: dict[str, str]) -> None:
        ...
        req_json = {
            "entityTag": [
                {"entityId": int(self.organization_id), "entityType": 61, "tag": tag_list}
            ]
        }
        self._commcell_object.wrap_request(
            "PUT",
    ...
    def isAuthrestoreEnabled(self) -> bool | None:
        ...
        """
        return (
            self.organization_properties.get("advancedPrivacySettings", {})
            .get("passkeySettings", {})
            .get("enableAuthorizeForRestore")
        )
            ...
            if response.json():
                if "error" in response.json():
                    ...
                    if error_code != 0:
                        ...
                        raise SDKException("Organization", "102", f"Error: {error_message}")
                theme = (
                    response.json().get("organizationInfo", {}).get("organizationProperties", {})
                )
    ...
    def company_theme(self, theme_colors: dict) -> None:
        ...
        request_json = {
            "organizationInfo": {
                "organizationProperties": {"customization": json.dumps(theme_colors)},
            }
        }
        flag, response = self._cvpysdk_object.make_request(
    ...
    def passkey(self, current_password: str, action: str, new_password: str | None = None) -> None:
        ...
        if action.lower() == "enable":
            ...
            raise SDKException(
                "Organization", 102, "Action is undefined, Invalid action passed as a parameter"
            )
    ...
    def update_general_settings(self, general_settings_dict: dict) -> None:
        ...
        flag, response = self._cvpysdk_object.make_request(
            "PUT",
            self._services["EDIT_COMPANY_DETAILS"] % self.organization_id,
    ...
    def geo_info(self) -> tuple[str | None, list[str]]:
        ...
        """
        geo_info = self.organization_info.get("organizationProperties", {}).get(
            "companyGeoInfo", []
        )
        workloads = []
        idp = None
    ...
    def extend(
        self, region: str, country: str, source_commcell: str, target_commcell: str
    ) -> None:
        """
        ...
        """
        req_json = {
            "sourceCommcell": {"commcell": {"commCellName": source_commcell}},
            "targetCommcell": {
                "commcell": {"commCellName": target_commcell},
                "companyGeoInfo": {"country": country, "companyRegion": {"regionName": region}},
            },
            "tenantCompany": {"providerGUID": self.provider_guid.lower()},
        }
