...
class VirtualMachinePolicies:
    ...
    def __str__(self):
        ...
        for index, vm_policy in enumerate(self._vm_policies):
            sub_str = f"{index + 1:^5}\t{vm_policy:20}\n"
        ...
        if flag:
            if response.json() and "policy" in response.json():
                ...
                for vm_policy in vm_policies:
                    ...
                    vm_policies_dict[temp_name] = {"id": temp_id, "policyType": temp_policy_type}
    ...
    def _set_vclient_and_vcenter_names(self, vm_policy_options, vclient_name):
        ...
        if vclient_name in vclient_name_dict:
            ...
            err_msg = f'Virtualization client "{vclient_name}" does not exist'
            raise SDKException("Virtual Machine", "102", err_msg)
    ...
    def _get_proxy_client_json(self, options):
        try:
            ...
            return dict()
        return {"clientId": int(id_), "clientName": options["proxy_client"]}
    ...
    def _prepare_add_vmpolicy_json_default(self, vm_policy_options):
        ...
        _esxservers = [
            {"esxServerName": esx_server} for esx_server in vm_policy_options.get("esxServers", "")
        ]
        _datastores = [
            {"dataStoreName": datastore} for datastore in vm_policy_options.get("dataStores", "")
        ]
        ...
        _vm_policy_json = {
            ...
            "policy": {
                ...
                "destinationHyperV": {
                    "clientId": int(
                        self._commcell_object.clients[vm_policy_options["clientName"]]["id"]
                    ),
    ...
    def _set_data_center(self, vm_policy_options):
        ...
        get_datacenter_xml = (
            ...
            'vmAllocPolicyName="" vCenterName="'
            + vm_policy_options["vCenterName"]
            ...
            if response.json():
                if "error" in response.json():
                    if response.json()["error"]["errorCode"] != 0:
                        ...
                        o_str = (
                            f'Failed to create virtual machine policy\nError: "{error_message}"'
                        )
                ...
                return VirtualMachinePolicy(
                    ...
                    int(
                        self._vm_policies[vm_policy_json["policy"]["entity"]["vmAllocPolicyName"]][
                            "id"
                        ]
                    ),
                )
    ...
    def _security_associations_json(vm_policy_options):
        ...
        if "users" in vm_policy_options:
            ...
            # default - admin
            default_user = {"_type_": 13, "userGUID": "admin", "userName": "admin", "userId": 1}
        ...
        if "mediaAgent" in vm_policy_options:
            ...
            if not self._commcell_object.media_agents.has_media_agent(media_agent):
                raise SDKException(
                    "Virtual Machine",
                    "102",
                    f'No media agent exists "{media_agent}" exists in commserv "{self._commcell_object.commserv_name}"',
                )
            else:
    ...
    def has_policy(self, vm_policy_name):
        ...
        return self._vm_policies and vm_policy_name.lower() in self._vm_policies
    ...
    def get(self, vm_policy_name):
        ...
        if self.has_policy(vm_policy_name):
            ...
            raise SDKException(
                "Virtual Machine", "102", f"No policy exists with name: {vm_policy_name}"
            )
    ...
    def add(self, vm_policy_name, vm_policy_type, vclient_name, vm_policy_options=None):
        ...
        _vm_policy_types = {"live mount": 4, "clone from template": 0, "restore from backup": 13}
        self.refresh()
        if (
            ...
            err_msg = f"{vm_policy_type} is not a valid virtual machine policy type."
            raise SDKException("Virtual Machine", "102", err_msg)
        elif self.has_policy(vm_policy_name):
            err_msg = (
                f'Virtual Machine Policy "{vm_policy_name}" already exists (not case sensitive)'
            )
            raise SDKException("Virtual Machine", "102", err_msg)
        else:
            ...
            if flag:
                if response.json():
                    if "error" in response.json():
                        if response.json()["error"]["errorCode"] != 0:
                            error_message = response.json()["error"]["errorMessage"]
                            o_str = f'Failed to create virtual machine policy\nError: "{error_message}"'
                            raise SDKException("Virtual Machine", "102", o_str)
    ...
    def delete(self, vm_policy_name):
        ...
        if self.has_policy(vm_policy_name):
            ...
            policy_delete_url = self._VMPOLICIES_URL + f"/{vm_policy_id}"
...
class VirtualMachinePolicy:
    ...
    def __new__(cls, commcell_object, vm_policy_name, vm_policy_type_id, vm_policy_id=None):
    ...
    def __init__(self, commcell_object, vm_policy_name, vm_policy_type_id, vm_policy_id=None):
        ...
        self._VM_POLICY_URL = (
            self._commcell_object._services["GET_VM_ALLOCATION_POLICY"] % self._vm_policy_id
        )
    ...
    def __repr__(self):
        ...
        return (
            f"VirtualMachinePolicy class instance for Virtual Machine Policy: '{self.vm_policy_name}' for "
            f"Commcell: '{self._commcell_object.commserv_name}'"
        )
            ...
            if response.json():
                if "error" in response.json():
                    if response.json()["error"]["errorCode"] != 0:
                        error_message = response.json()["error"]["errorMessage"]
                        o_str = (
                            f'Failed to update virtual machine policy\nError: "{error_message}"'
                        )
                        raise SDKException("Virtual Machine", "102", o_str)
            else:
                raise SDKException("Response", "102")
    ...
    def clone(self, desired_vm_policy_name):
        ...
        if not isinstance(desired_vm_policy_name, str):
            ...
            err_msg = f'Policy "{desired_vm_policy_name}" already exists'
            raise SDKException("Virtual Machine", "102", err_msg)
        else:
            import copy
            ...
            desired_vm_policy_json = {"action": 0, "policy": desired_vm_policy_properties}
...
class LiveMountPolicy(VirtualMachinePolicy):
    ...
    def __init__(self, commcell_object, vm_policy_name, vm_policy_type_id, vm_policy_id=None):
        ...
        """
        super().__init__(commcell_object, vm_policy_name, vm_policy_type_id, vm_policy_id)
    ...
    def _subtask_json():
        ...
        """
        _subtask = {"subTaskType": 1, "operationType": 4038}
    ...
    def _csinfo_json(live_mount_options):
        ...
        _csinfo = {
            ...
            "ip": {},
            "commservInfo": {},
            "creds": {"password": "", "domainName": "", "confirmPassword": "", "userName": ""},
        }
    ...
    def _vm_info_json(live_mount_options):
        ...
        _vm_info = {
            "advancedProperties": {
                "networkCards": [{"label": live_mount_options.get("network_name", "")}]
            },
            "vm": {"vmName": live_mount_options["vmName"], "_type_": 88},
        }
    ...
    def live_mount(self, client_vm_name, live_mount_options=None):
        ...
        # check if client is a valid hidden client
            ...
            err_msg = f'Client "{client_vm_name}" not found in Commcell'
            raise SDKException("Virtual Machine", "102", err_msg)
        else:
            ...
            if flag:
                if response.json():
                    if "error" in response.json():
                        if response.json()["error"]["errorCode"] != 0:
                            error_message = response.json()["error"]["errorMessage"]
                            o_str = f'Failed to run Live Mount\nError: "{error_message}"'
                            raise SDKException("Virtual Machine", "102", o_str)
                    # if no valid error in response
                    if "jobIds" in response.json():
                        return Job(self._commcell_object, response.json()["jobIds"][0])
                    else:
                        raise SDKException("Virtual Machine", "102", "Failed to run live mount")
                else:
                    raise SDKException("Response", "102")
            else:
                response_string = self._commcell_object._update_response_(response.text)
