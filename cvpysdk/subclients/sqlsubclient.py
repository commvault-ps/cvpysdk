...
class SQLServerSubclient(DatabaseSubclient):
    ...
    def _get_subclient_properties(self):
        ...
        self._is_file_group_subclient = (
            self._mssql_subclient_prop.get("sqlSubclientType", False) == 2
        )
    ...
    def content(self, subclient_content):
        ...
        if self._is_file_group_subclient:
            ...
            for database_name in subclient_content:
                sql_server_dict = {"mssqlDbContent": {"databaseName": database_name}}
    ...
    def browse(self):
        raise AttributeError(
            "'{0}' object has no attribute '{1}'".format(self.__class__.__name__, "browse")
        )

    @property
    def browse_in_time(self):
        raise AttributeError(
            "'{0}' object has no attribute '{1}'".format(self.__class__.__name__, "browse_in_time")
        )
    ...
    def restore_in_place(self):
        raise AttributeError(
            "'{0}' object has no attribute '{1}'".format(
                self.__class__.__name__, "restore_in_place"
            )
        )
    ...
    def restore_out_of_place(self):
        raise AttributeError(
            "'{0}' object has no attribute '{1}'".format(
                self.__class__.__name__, "restore_out_of_place"
            )
        )

    def backup(self, backup_level="Differential", data_options=[], schedule_pattern=None):
        ...
        if data_options or schedule_pattern:
            if data_options:
                ...
                if "checksum" in data_options and "use_sql_compression" in data_options:
                    raise ValueError(
                        "checksum or use_sql_compression can be enabled , but not both"
                    )
                if backup_level == "full" and any(
                    option in data_options for option in invalid_full_data_opts
                ):
                    raise ValueError(
                        f"{invalid_full_data_opts} are not applicable for full backup"
                    )
                elif backup_level == "transaction_log" and any(
                    option in data_options for option in invalid_transaction_log_data_opts
                ):
                    raise ValueError(
                        f"{invalid_transaction_log_data_opts} are not applicable for Transaction log backup"
                    )
                elif backup_level == "differential" and any(
                    option in data_options for option in invalid_differential_data_opts
                ):
                    raise ValueError(
                        f"{invalid_differential_data_opts} are not applicable for full backup"
                    )

                advanced_options["dataOpt"] = {
                    "enableIndexCheckPointing": False,
                    "verifySynthFull": True,
                    "startLogBackupAfterSuccessfullBackup": "start_log_backup_after_successfull_backup"
                    in data_options,
            ...
            request_json = self._backup_json(
                backup_level, False, "BEFORE_SYNTH", advanced_options, schedule_pattern
            )
    ...
    def update_content(self, subclient_content, action):
        ...
        content_op_dict = {"contentOperationType": action}
    ...
    def blocklevel_backup_option(self):
        ...
        """
        return bool(
            self._subclient_properties.get("mssqlSubClientProp", {}).get(
                "useBlockLevelBackupWithOptimizedRecovery", False
            )
        )
    ...
    def blocklevel_backup_option(self, value):
        ...
        self._set_subclient_properties(
            "_mssql_subclient_prop['useBlockLevelBackupWithOptimizedRecovery']", value
        )
