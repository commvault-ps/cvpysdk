...
class ExchangeSubclient(Subclient):
    ...
    def _json_job_option_items(self, value):
        ...
        match_destination_user = value.get(
            "match_destination_user", JobOptionValues.DISABLED.value
        )
        stub_rehydration_option = value.get(
            "stub_rehydration_option", JobOptionValues.RECOVER_STUBS.value
        )
        ...
        self._job_option_items_json = [
            {
                "option": JobOptionKeys.RESTORE_DESTINATION.value,
                "value": JobOptionValues.EXCHANGE.value,
            },
            {
                "option": JobOptionKeys.DESTINATION.value,
                "value": JobOptionValues.ORIGINAL_LOCATION.value,
            },
            ...
            {
                "option": JobOptionKeys.MATCH_DESTINATION_USER.value,
                "value": match_destination_user,
            },
        ...
        if stub_rehydration != JobOptionValues.DISABLED.value:
            if stub_rehydration_option == JobOptionValues.RECOVER_STUBS.value:
                ...
                additional_options = [
                    {
                        "option": JobOptionKeys.MAILBOX_LEVEL_REPORTING.value,
                        "value": JobOptionValues.ENABLED.value,
                    },
                    {
                        "option": JobOptionKeys.EMAIL_LEVEL_REPORTING.value,
                        "value": email_level_reporting,
                    },
                ]
    ...
    def _exchange_option_restore_json_rehydration(self, value):
        ...
        if stub_rehydration_option == JobOptionIntegers.RECOVER_STUBS.value:
            ...
            additional_options = {"stubOldRecallLink": old_link, "stubRecallLink": new_link}
    ...
    def _json_content_indexing_subtasks(self):
        ...
        _subtask_restore_json = {"subTaskType": 1, "operationType": 5020}
                ...
                "subTasks": [
                    {
                        ...
                        "options": {"backupOpts": {"backupLevel": 15}},
                    }
                ],
            }
        }
        ...
        **kwargs,
        ...
        if restore_as_stub:
            request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                "commonOptions"
            ] = self._json_restore_exchange_common_option(restore_as_stub)

        stub_rehydration = kwargs.get("stub_rehydration")
        if stub_rehydration:
            request_json["taskInfo"]["subTasks"][0]["options"]["commonOpts"]["jobOptionItems"] = (
                self._json_job_option_items(stub_rehydration)
            )
            request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                "exchangeOption"
            ] = self._exchange_option_restore_json_rehydration(stub_rehydration)
        ...
        if filters:
            if filters.get("restore_all_matching"):
                request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                    "exchangeOption"
                ]["restoreAllMatching"] = filters.get("restore_all_matching", False)
                request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"]["fileOption"][
                    "browseFilters"
                ] = [self._browse_filter_xml(filters)]
    ...
    def out_of_place_restore(self, paths, destination_path, overwrite=True, journal_report=False):
        """Restores the mailboxes/folders specified in the input paths list to the same location.
    ...
    def disk_restore(
        self, paths, destination_client, destination_path, overwrite=True, journal_report=False
    ):
        """Restores the mailboxes/folders specified in the input paths list to the same location.
    ...
    def pst_restore(
        self, paths, destination_client, pst_path, overwrite=True, journal_report=False
    ):
        ...
        payload_dict = {
            "taskInfo": {
                ...
                "subTasks": [
                    {
                        "subTaskOperation": 1,
                        "subTask": {"subTaskType": 2, "operationType": 5024},
                        "options": {
    ...
    def subclient_content_indexing(
        self, pick_failed_items=False, pick_only_failed_items=False, streams=4, proxies=None
    ):
        ...
        if proxies is None:
            ...
            ci_option["proxies"] = {"memberServers": member_servers}
                ...
                "subTasks": [
                    {
                        ...
                        "options": {
                            "backupOpts": {"mediaOpt": self._media_option_json_},
                            "adminOpts": {
    ...
    def _prepare_attachment_json(self, metadata, attachment_id):
        ...
        request_json = {
            "filters": [
                ...
                {"field": "APP_TYPE", "fieldValues": {"values": ["137"]}},
                {
                    "field": "CONTENTID",
                    "fieldValues": {
                        "values": [
                            str(
                                metadata["advanced_data"]["browseMetaData"]["indexing"][
                                    "objectGUID"
                                ]
                            )
                        ]
                ...
                {"field": "ATTACHMENTID", "fieldValues": {"values": [attachment_id]}},
                {
                    "field": "CV_TURBO_GUID",
                    "fieldValues": {
                        "values": [
    ...
    def _get_ad_group_backup_common_opt_json(ad_group_name):
        ...
        return {
            ...
            "jobMetadata": [
                {
                    "selectedItems": [{"itemName": ad_group_name, "itemType": "AD group"}],
    ...
    def _get_ad_group_backup_backupoptions_json(ad_group_name):
        ...
        return {
            "backupLevel": 2,
            "incLevel": 1,
            "exchOnePassOptions": {"adGroups": [{"adGroupName": ad_group_name}]},
            "dataOpt": {
                "useCatalogServer": False,
                "followMountPoints": True,
                "enforceTransactionLogUsage": False,
