...
from ..constants import SharepointDefines, SQLDefines
...
class SharepointSuperSubclient(Subclient):
    ...
    def _get_subclient_properties(self):
        ...
        self._sharepoint_subclient_prop = self._subclient_properties.get(
            "sharepointsubclientprop", {}
        )
...
class SharepointSubclient(SharepointSuperSubclient):
    ...
    def content(self, subclient_content):
        ...
        for webapp in subclient_content:
            sp_server_dict = {"spContentPath": webapp}
            content.append(sp_server_dict)

        self._set_subclient_properties("_content", content)

    def restore(self, content_to_restore, database_client, spsetup_list, overwrite=True):
        ...
        if not self._backupset_object.is_sharepoint_online_instance:
            ...
            request_json = self._sharepoint_restore_options_json(
                content_to_restore, database_client, spsetup_list, overwrite=overwrite
            )

            return self._process_restore_response(request_json)
        else:
            raise SDKException(
                "Subclient", "102", "Method not supported for SharePoint Online Instance"
            )

    def _sharepoint_restore_options_json(
        self, content, database_client, spsetup_list, destination_client=None, overwrite=None
    ):
        ...
        if not self._backupset_object.is_sharepoint_online_instance:
            ...
            request_json = self._restore_json(client=client_name)

            if destination_client is None:
                destination_client = client_name

            common_options = {"allVersion": True, "erExSpdbPathRestore": True}
            destination = {
            ...
            sharepoint_restore_option = {
                ...
                "spRestoreToDisk": {"restoreToDisk": False},
            }
            sharepoint_db_restore_option = {
                "restoreType": "SameConfiguration",
                "restoreDatabaseOption": "RESTORE_ALL",
                "rbsOptions": {
                    "sqlSource": {"clientName": database_client},
                    "sqlDestination": {"destClient": {"clientName": database_client}},
                },
            }
            ...
            request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                "sharePointDBRestoreOption"
            ] = sharepoint_db_restore_option
            request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                "commonOptions"
            ].update(common_options)
            request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                "destination"
            ].update(destination)
            request_json["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
                "sharePointRstOption"
            ].update(sharepoint_restore_option)
            ...
            for item in content:
                ...
                for sp_dict in spsetup_list:
                    if sp_dict["application_pool"].lower() == item.lower():
                        ...
                        source_items.append(SharepointDefines.CONTENT_WEBAPP.format(item))
                        source_items.append(
                            ...
                            "SharePointMetaData": [
                                {
                                    ...
                                    "sourceItem": SharepointDefines.CONTENT_DB.format(
                                        item, database_name
                                    ),
                                ...
                                {
                                    "sourceItem": SharepointDefines.CONTENT_WEBAPP.format(item),
    ...
    def _set_properties_to_update_site_association(self, operation):
        ...
        if self._backupset_object.is_sharepoint_online_instance:
            ...
            raise SDKException(
                "Subclient", "102", "Method not supported for SharePoint On-Premise Instance"
            )
    ...
    def add_azure_app(
        ...
        azure_app_list = (
            properties_dict["sharepointBackupSet"]["spOffice365BackupSetProp"]
            .get("azureAppList", {})
            .get("azureApps", [])
        )
    ...
    def delete_azure_app(self, app_ids):
        ...
        properties_dict = self._backupset_object.properties
        azure_app_list = properties_dict["sharepointBackupSet"]["spOffice365BackupSetProp"][
            "azureAppList"
        ]["azureApps"]
        new_app_list = []

        for azure_app_dict in azure_app_list:
            if azure_app_dict["azureAppId"] not in app_ids:
    ...
    def run_manual_discovery(self):
        ...
        if self._backupset_object.is_sharepoint_online_instance:
            ...
            flag, response = self._cvpysdk_object.make_request("GET", self._MANUAL_DISCOVERY)
            if flag:
                if response.json():
                    if "response" in response.json():
                        ...
                        if response:
                            ...
                            if error_code != 0:
                                ...
                                o_str = f'Failed to run manual discovery\nError: "{error_string}"'
            ...
            request_json = {
                "cloudAppAssociation": {
                    ...
                    "cloudAppDiscoverinfo": {
                        ...
                        "groups": [{"name": association_group_name}],
                    },
                    "plan": {"planId": plan_id},
                }
            }
                ...
                if response.json():
                    if "response" in response.json():
                        ...
                        if response:
                            ...
                            if error_code != 0:
                                error_string = response.json().get("response", {})
                                o_str = f'Failed to set \nError: "{error_string}"'
                                raise SDKException("Subclient", "102", o_str)
                    elif "errorMessage" in response.json():
                        error_string = response.json().get("errorMessage", "")
                        o_str = f'Failed to set category based content for association\nError: "{error_string}"'
                        raise SDKException("Subclient", "102", o_str)
            else:
                raise SDKException("Response", "101", self._update_response_(response.text))
        else:
    ...
    def update_auto_association_group_properties(
        self, discovery_type, association_group_name, account_status=None, plan_id=None
    ):
        ...
        if self._backupset_object.is_sharepoint_online_instance:
            ...
            if plan_id:
                request_json["cloudAppAssociation"]["plan"] = {"planId": plan_id}
            flag, response = self._cvpysdk_object.make_request(
                ...
                if response.json():
                    if "resp" in response.json():
                        ...
                        if error_code != 0:
                            ...
                            o_str = f'Failed to enable group\nError: "{error_string}"'
                            raise SDKException("Subclient", "102", o_str)
                    elif "errorMessage" in response.json():
            ...
            if flag:
                if response and response.json():
                    ...
                    if "associations" in response.json():
                        ...
                        no_of_records = (
                            response.json().get("pagingInfo", {}).get("totalRecords", -1)
                        )
                        if no_of_records <= 0:
    ...
    def update_sites_association_properties(
        self, site_user_accounts_list, operation, plan_id=None
    ):
        """Updates the association properties of site
        ...
        if self._backupset_object.is_sharepoint_online_instance:
            ...
            if properties_dict.get("accountStatus", None) is not None:
                request_json["cloudAppAssociation"]["accountStatus"] = properties_dict[
                    "accountStatus"
                ]
            if plan_id:
                request_json["cloudAppAssociation"]["plan"] = {"planId": plan_id}
            ...
            if flag:
                if response.json():
                    if "resp" in response.json():
                        ...
                        if error_code != 0:
                            error_string = (
                                response.json().get("response", {}).get("errorString", "")
                            )
                            o_str = f'Failed to associate content\nError: "{error_string}"'
                            raise SDKException("Subclient", "102", o_str)
                    elif "errorMessage" in response.json():
    ...
    def delete_data(
        self, guids=None, search_string=None, folder_delete=False, search_and_select_all=False
    ):
        """
                        ...
                        "filters": [
                            {
                                "field": "FILE_NAME",
                                "intraFieldOp": 0,
                                "fieldValues": {"values": [search_string, search_string + "*"]},
                            },
                            {
                                "field": "SPTitle",
                                "intraFieldOp": 0,
        ...
        request_json = {
            "opType": 1,
            "bulkMode": bulk_mode,
            "deleteOption": {"folderDelete": bulk_mode},
            "searchReq": {
                "mode": 4,
                "advSearchGrp": {
                    "commonFilter": [
                                ...
                                "filters": [
                                    {
                                        "field": "CISTATE",
                                        "intraFieldOp": 0,
                                        "fieldValues": {"values": ci_state_values},
                                    }
                                ],
                            }
                        }
    ...
    def restore_in_place_syntex(self, **kwargs):
        ...
        if self._backupset_object.is_sharepoint_online_instance:
                ...
                syntex_restore_items.append(
                    {
                        ...
                        "rawId": sharepoint_item["siteId"]
                        + ";"
                        + sharepoint_item["objectId"]
                        + ";"
                        + sharepoint_item["contentPath"],
                        "restoreType": 1,
                    }
                )
            ...
            parameter_dict["taskInfo"]["subTasks"][0]["options"]["restoreOptions"][
    ...
    def out_of_place_restore(
        self, paths, destination_path, overwrite=True, common_options: dict = None
    ):
    ...
    def blob_restore(
        ...
        if self._backupset_object.is_sharepoint_online_instance:
            ...
            if credential is None:
                raise SDKException(
                    "Subclient", "102", f'Credential "{destination_blob}" not found'
                )
            restore_option = {
                "paths": paths,
...
class SharepointV1Subclient(SharepointSuperSubclient):
    ...
    def discover_sharepoint_sites(self, paths):
            ...
            "advOptions": {
                "advConfig": {
                    "applicationMining": {
                        ...
                        "agentVersion": 2013,
                        "browseReq": {"spBrowseReq": {"spBrowseType": 2, "spBrowseLevel": 1}},
                    }
                }
            },
        }
        ...
        if flag:
            if response and response.json():
                ...
                if (
                    len(
                        response["browseResponses"][0]["browseResult"]["advConfig"][
                            "applicationMining"
                        ]["browseResp"]["spBrowseResp"]["dataPathContents"]
                    )
                    > 0
                ):
                    return response["browseResponses"][0]["browseResult"]["advConfig"][
                        "applicationMining"
                    ]["browseResp"]["spBrowseResp"]["dataPathContents"]
                else:
    ...
    def out_of_place_restore(self, paths, destination_path, overwrite=True):
        """Restores the SharePoint list/libraries specified in the input paths list to the different site
