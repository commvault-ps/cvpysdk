...
class ADSubclient(Subclient):
        ...
        if len(subclient_content) > 0:
            for entry in subclient_content:
                if len(entry["path"].split(",")) > 1:
                    ...
                    raise SDKException("Subclient", "101", "subclient content is not valid")
        else:
            raise SDKException("Subclient", "101", "subclient content return empty result")
    ...
    def cv_contents(self, contents, entrypoint=None):
        ...
        for content in contents:
            ...
            if entrypoint is not None:
                shortdn = contententry.split(f",{entrypoint}")[0]
            else:
                shortdn = None
    ...
    def trigger_compare_job(
        ...
        flag, response = self._cvpysdk_object.make_request(
            method="POST", url=trigger_compare_job, payload=payload
        )
    ...
    def checkcompare_result_generated(self, comp_id):
        ...
        while True:
            flag, response = self._cvpysdk_object.make_request(method="GET", url=compare_result)
            ...
            if flag:
                ...
                if error_message != "":
                    raise SDKException(
                        "Response", "101", self._commcell_objectj._update_response_(response.text)
                    )
    ...
    def generate_compare_report(self, comp_id, comparison_cache_path, op_type=2):
        ...
        payload = {
            "isEnterprise": True,
            "discoverySentTypes": [24],
            "subclientDetails": {
                "subclientId": (subclient_id),
                "appTypeId": 41,
                "clientId": (client_id),
            },
            "adComparisonBrowseReq": {
                "pageNumber": 0,
                "comparisonId": comp_id,
                "opType": op_type,
                "browseItemId": 0,
                "pageSize": 15,
                "comparisonCachePath": comparison_cache_path,
                "searches": [{"masks": ["2", "1", "0"], "searchType": 2}],
            },
        }

        flag, response = self._cvpysdk_object.make_request(
            method="POST", url=ad_comp_url, payload=payload
        )
    ...
    def restore_job(self, display_name, client_name, subclient_name, to_time, restore_path):
                ...
                "subTasks": [
                    {
                        ...
                        "options": {
                            "commonOpts": {
                                ...
                                "jobMetadata": [
                                    {
                                        "selectedItems": [{"itemName": "", "itemType": ""}],
                                        "jobOptionItems": [],
                                    }
                                ],
                            },
        ...
        flag, response = self._cvpysdk_object.make_request(
            method="POST", url=trigger_response, payload=payload
        )
        if not flag:
            raise SDKException(
                "Response", "101", self._commcell_object._update_response_(response.text)
