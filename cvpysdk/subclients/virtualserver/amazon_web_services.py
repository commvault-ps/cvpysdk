...
class AmazonVirtualServerSubclient(VirtualServerSubclient):
    ...
    def full_vm_restore_out_of_place(
        ...
        **kwargs,
        ...
        if vm_display_name:
            if not (isinstance(vm_to_restore, str) or isinstance(vm_display_name, str)):
                raise SDKException("Subclient", "101")
    ...
    def attach_disk_restore(
        ...
        **kwargs,
        ...
        instance_dict = self._backupset_object._instance_object._properties["instance"]
        _attach_disk_restore_option = self.amazon_defaults(
            vm_to_restore, _attach_disk_restore_option
        )
