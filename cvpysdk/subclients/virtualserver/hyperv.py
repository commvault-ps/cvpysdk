...
class HyperVVirtualServerSubclient(VirtualServerSubclient):
    ...
    def _get_guest_os(self, os_name):
        ...
        _os_name_dict = {"2012": "windows8Server64Guest"}
    ...
    def disk_restore(
        ...
        if not (isinstance(destination_path, str) and isinstance(vm_name, (str, list))):
        ...
        **kwargs,
        ...
        if bool(restore_option):
            if not (
                ...
                and isinstance(power_on, bool)
                and isinstance(add_to_failover, bool)
            ):
                raise SDKException("Subclient", "101")
    ...
    def full_vm_restore_in_place(
        ...
        **kwargs,
        ...
        ]["diskLevelVMRestoreOption"]["esxServerName"] = restore_option["esx_server"]
        return self._process_restore_response(request_json)
    ...
    def full_vm_conversion_vmware(
        ...
        # check mandatory input parameters are correct
        if not (isinstance(vcenter_client, str) and isinstance(destination_network, str)):
            raise SDKException("Subclient", "101")
