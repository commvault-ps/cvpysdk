...
class BLRPairs:
    ...
    def __repr__(self):
        ...
        if not self._replication_group_name:
            return f"BLR Pairs for Commserv: '{self._commcell_object.commserv_name}'"
        else:
            return f"BLR Pairs for Replication group: '{self._replication_group_name}'"
    ...
    def _get_replication_group_id(self):
        ...
        if not self._replication_group_name:
            return ""
        return self._commcell_object.replication_groups.replication_groups.get(
            self._replication_group_name, {}
        ).get("id")
    ...
    def _update_rpstorelist(self):
        ...
        Raises:
            SDKException:
                ...
                if response is not success
        """
        response = self._commcell_object.qoperation_execute(
            '<EVGui_GetLibraryListWCReq libraryType="RPSTORE" />'
        )
    ...
    def has_blr_pair(self, source_name, destination_name):
        ...
        for _, pair_row in self.blr_pairs.items():
            if (
                source_name == pair_row.get("sourceName", "").lower()
                and destination_name == pair_row.get("destinationName", "").lower()
            ):
                return True
        else:
            return False

    def create_fsblr_pair(
        self,
        source_client_id,
        destination_client_id,
        source_volumes,
        ...
        **kwargs,
        ...
        flag, response = self._commcell_object._cvpysdk_object.make_request(
            "POST", self._services["CREATE_BLR_PAIR"], request_json
        )
    ...
    def get(self, source_name, destination_name):
        ...
        for _, pair_row in self.blr_pairs.items():
            ...
            raise SDKException(
                "BLRPairs",
                "102",
                f'No BLR pair exists with source: "{source_name}" and destination: "{destination_name}"',
            )
                ...
                if (
                    ...
                    if flag:
                        if "error" in response.json():
                            ...
                            o_str = f'Failed to delete Source: {source_name} and Destination: {destination_name} \nError: "{error_message}"'
    ...
    def get_rpstore_id(self, rpstore_name):
        ...
        for rpstore in self._rpstore_list:
            if rpstore.get("library", {}).get("libraryName") == rpstore_name and rpstore.get(
                "MountPathList", ""
            ):
                return str(
                    rpstore.get("MountPathList")[0]
                    .get("rpStoreLibraryInfo", {})
                    .get("rpStoreId", "")
                )
        else:
...
class BLRPair:
    ...
    def _get_pair_id(self):
        ...
        for pair_id, blr_pair in self._commcell_object.blr_pairs.blr_pairs.items():
            if (
                blr_pair.get("sourceName").lower() == self._source_name
    ...
    def _perform_action(self, operation_type):
        ...
        flag, response = self._commcell_object._cvpysdk_object.make_request(
            "PUT", self._services["GET_BLR_PAIRS"], {"siteInfo": site_info}
        )
        if flag:
            if response.json():
    ...
    def pair_recovery_type(self):
        ...
        return BLRPairs.RecoveryType(
            self.pair_properties.get("blrRecoveryOpts", {}).get("recoveryType")
        )
    ...
    def get_pair_stats(self):
        ...
        """
        flag, response = self._commcell_object._cvpysdk_object.make_request(
            "GET", self._PAIR_STATS % self._pair_id
        )
        if flag:
                                                ...
                                                "mountPathPairs": [
                                                    {
                                                        ...
                                                        "srcPath": destination["accessPathList"][
                                                            0
                                                        ],
...
class BLRTestFailovers:
    ...
    def refresh(self):
        ...
        self._boot_vm_dict = {
            boot_dict.get("name"): boot_dict for boot_dict in self._get_test_failover_vms()
        }
    ...
    def get_boot_vm_details(self, vm_name):
        ...
        if not vm_dict:
            raise SDKException(
                "BLRPairs", "102", f"BLR test boot VM with name {vm_name} not found"
            )
        return {
            "id": vm_dict.get("id"),
