...
from .dr_orchestration_job_phase import DRJobPhases, DRJobPhaseToText


class DROrchestrationOperations:
    ...
    def _json_dr_orchestration_subtasks(self):
        ...
        _backup_subtask = {"subTaskType": 1, "operationType": 4046}
    ...
    def _json_dr_orchestration(self):
        ...
        initiatedFrom = self.dr_orchestration_options.get("initiatedfromMonitor", False)
        ...
        _dr_orchestration_json = {
            ...
            "advancedOptions": {
                "skipDisableNetworkAdapter": self.dr_orchestration_options.get(
    ...
    def force_reverse_replication(self):
        ...
        dr_orchestration_json["taskInfo"]["subTasks"][0]["options"]["adminOpts"][
            "drOrchestrationOption"
        ]["advancedOptions"].update({"powerOnVM": False})
    ...
    def point_in_time_failover(self, timestamp, replication_id):
        ...
        dr_orchestration_json["taskInfo"]["subTasks"][0]["options"]["adminOpts"][
            "drOrchestrationOption"
        ]["replicationInfo"]["configOption"] = [
            {"pointInTime": int(timestamp), "replicationId": int(replication_id)}
        ]
    ...
    def validate_dr_orchestration_job(self, jobId):
        ...
        _replicationIds = self.dr_orchestration_options.get("replicationIds", [])
            ...
            if dr_orchestration_job_stats_json and isinstance(
                ...
                if "phase" in dr_orchestration_job_stats_json:
                    for phase in dr_orchestration_job_stats_json["phase"]:
                        phase_type = DRJobPhaseToText(
                            self.dr_orchestration_job_phase(phase["phase"])
                        ).value
                        ...
                        if phase_state == 1:
                            ...
                            raise SDKException("DROrchestrationOperations", "102", o_str)

                else:
                    o_str = f"Failed to finish any phases in DR orchestration Job {jobId} \n"
                    raise SDKException("DROrchestrationOperations", "102", o_str)
    ...
    def get_snapshot_list(self, guid: str, instance_id: int, timestamp_filter: bool = True):
        ...
        """
        vm_browse_req = f"{self._commcell_object._web_service}VMBrowse/{guid}?instanceId={instance_id}&snapshots=true"
        ...
        if timestamp_filter:
            ...
            snapshots = list(filter(lambda s: "__GX_Recovery_Point_" in s["name"], snapshots))
                ...
                "subTasks": [
                    {
                        ...
                        "options": {
                            "adminOpts": {"drOrchestrationOption": self._json_dr_orchestration}
                        },
                    }
                ],
            }
    ...
    def _construct_reverse_replication_json(self):
        ...
        Raises:
            SDKException:
                ...
                if response is not success
        """
        dr_orchestration_json = {"drOrchestrationOption": self._json_dr_orchestration}
            ...
            if response.json():
                if "error" in response.json():
                    ...
                    o_str = 'Failed to start {0} job \nError: "{1}"'.format(
                        self._get_dr_orchestration_operation_string(
                            self.dr_orchestration_options.get("DROrchestrationType")
                        ),
                        error_message,
                    )
    ...
    def _get_dr_orchestration_job_stats(self, jobId, replicationId):
        ...
        if not isinstance(jobId, str) and not isinstance(replicationId, str):
            raise SDKException("DROrchestrationOperations", "101")
        ...
        if flag:
            if response.json():
                if "error" in response.json():
                    error_message = response.json()["error"]["errorMessage"]
                    o_str = f'Failed to validate DR orchestration job {jobId} \nError: "{error_message}"'
                    ...
                    if "job" in response.json():
                        ...
                        raise SDKException(
                            "DROrchestrationOperations",
                            "102",
                            "Failed to start DR orchestration job",
                        )
