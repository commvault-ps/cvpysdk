...
class ComplianceSearchUtils:
    ...
    def do_compliance_search(
        ...
        if index_server_name:
            ...
            search_request["listOfCIServer"][0]["cloudID"] = self._index_servers.get(
                index_server_name
            ).cloud_id
        else:
            del search_request["listOfCIServer"]
        search_request["advSearchGrp"]["cvSearchKeyword"]["keyword"] = search_text
        search_request["userInformation"]["userGuid"] = self._users.get(
            self._commcell.commcell_username
        ).user_guid
        if app_type in ComplianceConstants.FILE_TYPES:
            search_request["advSearchGrp"]["galaxyFilter"][0]["applicationType"] = (
                ComplianceConstants.FILE_TYPE
            )
            search_request["advSearchGrp"][ComplianceConstants.FILE_FILTERS_KEY] = (
                ComplianceConstants.FILE_FILTERS
            )
            search_request["advSearchGrp"][ComplianceConstants.COMMON_FILTER] = (
                ComplianceConstants.COMMON_FILTERS
            )
        ...
        if page_size == 0:
            flag, response = self._cvpysdk_object.make_request(
                "POST", self._do_search_api, payload=search_request
            )
...
class ExportSets:
        ...
        if flag:
            if response.json() and "containers" in response.json():
                ...
                for container in containers:
                    self._all_export_set.update({container["containerName"]: container})
        else:
            self._response_not_success(response)
    ...
    def refresh(self):
        ...
        self._delete_export_set_json_req = {"entityType": 9503, "containers": None}
    ...
    def delete(self, export_set_name):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "POST", self._delete_export_set_api, delete_json
        )
        if flag:
            if (
                response.json()
                and "errList" in response.json()
                and len(response.json()["errList"]) == 0
            ):
                self.refresh()
                return True
        self._response_not_success(response)
...
class ExportSet:
    ...
    def refresh(self):
        ...
        self._export_items_json_req = {
            "complianceData": {
                ...
                "downLoadDesc": None,
                "destContainer": None,
                "originatingContainer": {"containerOwnerType": 1},
                "options": {"zipEML": True, "retentionInDays": 30},
            },
            "onlineData": {"downloadStatus": 0},
            "listOfItems": {"resultItem": None},
        }
        self._delete_export_api = self._services["GET_EXPORTS"]
        self._delete_export_json_req = {
            "operationType": 2,
            "container": None,
            "downloadItems": {"items": None},
            "tagModifyRequest": {"operationType": 2},
        }
        self._all_exports = {}
    ...
    def _get_all_exports(self):
        ...
        if flag:
            if (
                response.json()
                and "totalHits" in response.json()
                and int(response.json()["totalHits"]) != 0
            ):
                items = response.json()["downloadItems"]["items"]
                for item in items:
                    self._all_exports.update({item["description"]: item})
            else:
                self._all_exports = {}
        else:
            self._response_not_success(response)
    ...
    def share(self, user_or_user_group_name=None, permissions=None, mode=2):
        ...
        if user_or_user_group_name is None:
            ...
            user_details = {
                "groupId": int(
                    self._commcell.user_groups.get(user_or_user_group_name).user_group_id
                ),
        ...
        if unshare:
            ...
            if permissions is None:
                share_json["securityAssociations"]["associations"][0]["properties"][
                    "permissions"
                ] = []
                permissions = [["View", "Download"], list(ComplianceConstants.PERMISSIONS.keys())][
                    mode // 3
                ]
            if mode == 3:
                share_json["securityAssociations"]["associations"][0]["properties"][
    ...
    def delete(self, export_name):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "POST", self._delete_export_api, delete_json
        )
        if flag:
            if (
                response.json()
    ...
    def export_items_to_set(
        self, export_name, export_items, export_type=ComplianceConstants.ExportTypes.CAB
    ):
        ...
        export_items_json_req["complianceData"]["restoreType"] = ComplianceConstants.RESTORE_TYPE[
            export_type
        ]
...
class Export:
    ...
    def refresh(self):
        ...
        self._download_file_json_req = {
            ...
            "fileParams": [
                {"name": self._export_set_properties["downLoadID"], "id": 2},
                {"name": "zip", "id": 3},
            ],
        }
    ...
    def download_export(self, download_folder):
        ...
        if "cloudUrl" in response.json() or "data" in file_content:
            ...
            flag, response = self._cvpysdk_object.make_request(
                method="POST",
                url=self._download_via_stream,
                payload=self._download_file_json_req,
                stream=True,
            )
