...
from ..activateapps.constants import (
    EdiscoveryConstants,
    InventoryConstants,
    RequestConstants,
    TargetApps,
...
class EdiscoveryClients:
    ...
    def __init__(self, commcell_object, class_object):
        ...
        self._API_GET_EDISCOVERY_CLIENTS = copy.deepcopy(
            self._services["EDISCOVERY_V2_GET_CLIENTS"]
        )
        self._API_GET_EDISCOVERY_CLIENT_GROUPS = self._services[
            "EDISCOVERY_V2_GET_CLIENT_GROUP_DETAILS"
        ]
        ...
        from .file_storage_optimization import FsoServerGroup, FsoServerGroups, FsoServers
    ...
    def add(self, client_name, inventory_name, plan_name):
        ...
        if inventory_name is not None:
            if not self._commcell_object.activate.inventory_manager().has_inventory(
                inventory_name
            ):
            ...
            req_json["clientInfo"]["edgeDrivePseudoClientProperties"]["eDiscoveryInfo"][
                "inventoryDataSource"
            ]["seaDataSourceId"] = int(inv_obj.inventory_id)
        flag, response = self._cvpysdk_object.make_request(
    ...
    def get_ediscovery_client_group_details(self):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "GET",
            self._API_GET_EDISCOVERY_CLIENT_GROUPS
...
class EdiscoveryClientOperations:
    ...
    def __init__(self, commcell_object, class_object):
        ...
        self._API_GET_EDISCOVERY_CLIENT_DETAILS_V1 = copy.deepcopy(
            self._services["EDISCOVERY_CLIENT_DETAILS"]
        )
        ...
        if isinstance(class_object, FsoServer):
            ...
            self._sort_dir = 0
            self._ds_type_names = (
                f"{EdiscoveryConstants.DS_FILE},{EdiscoveryConstants.DS_CLOUD_STORAGE}"
            )
    ...
    def schedule(self, schedule_name, pattern_json, ops_type=2):
        ...
        if ops_type not in [2]:
            raise SDKException(
                "EdiscoveryClients", "102", "Schedule operation type provided is not supported"
            )
        ...
        request_json["taskInfo"]["task"]["taskName"] = (
            f"Cvpysdk created Schedule -{schedule_name} for Server id - {self._client_id}"
        )
        request_json["taskInfo"]["subTasks"][0]["subTask"]["subTaskName"] = schedule_name
        request_json["taskInfo"]["subTasks"][0]["pattern"] = pattern_json
        request_json["taskInfo"]["subTasks"][0]["options"]["adminOpts"]["contentIndexingOption"][
            "operationType"
        ] = ops_type
    ...
    def form_search_params(
        ...
        search_params["searchParams"].append({key: "wt", "value": "json"})
        search_params["searchParams"].append({key: "defType", "value": "edismax"})
        search_params["searchParams"].append({key: "q", "value": query})
        if criteria:
            fq_dict = {key: "fq", "value": criteria}
            search_params["searchParams"].append(fq_dict)
        if attr_list:
            if is_separate_attr:
                for attr in attr_list:
                    fl_dict = {key: "fl", "value": attr}
                    search_params["searchParams"].append(fl_dict)
            else:
                fl_list = ",".join(attr_list)
                fl_dict = {key: "fl", "value": fl_list}
                search_params["searchParams"].append(fl_dict)
        if params:
            for dkey, value in params.items():
                custom_dict = {key: str(dkey), "value": str(value)}
    ...
    def _do_stream_download(self, guid, file_name, download_location):
        ...
        if flag:
            ...
            if error_list:
                raise SDKException("EdiscoveryClients", "102", f"Error: {error_list}")
            ...
            flag1, response1 = self._cvpysdk_object.make_request(
                "POST", self._services["DOWNLOAD_VIA_STREAM"], request, stream=True
            )
        ...
        if flag:
            if response.json() and "handlerInfos" in response.json():
                ...
                if not isinstance(handler_list, list):
                    raise SDKException(
                        "EdiscoveryClients", "102", "Failed to get Datasource/Handler details"
                    )
                for handler in handler_list:
                    ...
                    raise SDKException(
                        "EdiscoveryClients", "102", "No Handler found with given name"
                    )
            raise SDKException(
                "EdiscoveryClients", "102", "Unknown response while fetching datasource details"
            )
        self._response_not_success(response)
    ...
    def export(self, criteria=None, attr_list=None, params=None):
        ...
        if criteria and not isinstance(criteria, str):
            raise SDKException("EdiscoveryClients", "101")
        search_params = self.form_search_params(
        ...
        if flag:
            if response.json():
                ...
                raise SDKException(
                    "EdiscoveryClients",
                    "102",
                    f"Failed to search with response - {response.json()}",
                )
    ...
    def search(self, criteria=None, attr_list=None, params=None):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "POST",
            self._API_SEARCH % (self._search_entity_type, self._search_entity_id),
            search_params,
        )
        if flag:
            if response.json():
    ...
    def _get_associations(self):
        ...
        if (
            isinstance(self._class_obj, Request)
            or isinstance(self._class_obj, EdiscoveryDatasource)
            or isinstance(self._class_obj, FsoServerGroup)
            or isinstance(self._class_obj, Projects)
        ):
            return {}
        ...
        association_request_json["entityAssociated"]["entity"][0]["clientId"] = int(
            self._client_id
        )
        flag, response = self._cvpysdk_object.make_request(
            "GET",
            self._API_SECURITY_ENTITY % (self._client_entity_type, int(self._client_id)),
        ...
        if flag:
            if response.json() and "securityAssociations" in response.json():
                ...
                raise SDKException(
                    "EdiscoveryClients", "102", "Failed to get existing security associations"
                )
        else:
            response_string = self._commcell_object._update_response_(response.text)
            raise SDKException("Response", "101", response_string)
    ...
    def share(self, user_or_group_name, allow_edit_permission=False, is_user=True, ops_type=1):
        ...
        if allow_edit_permission:
            ...
            association_json = copy.deepcopy(
                request_json["securityAssociations"]["associations"][0]
            )
            # do copy, remove permission and add Edit
            del association_json["properties"]["categoryPermission"]["categoriesPermissionList"][0]
            association_json["properties"]["categoryPermission"][
                "categoriesPermissionList"
            ].append(EdiscoveryConstants.EDIT_CATEGORY_PERMISSION)
                ...
                for node in response_json:
                    if "errorCode" in node:
                        ...
                        if error_code != 0:
                            ...
                            raise SDKException("EdiscoveryClients", "102", error_message)
                self.refresh()
            else:
    ...
    def get_ediscovery_client_details(self):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "GET",
            self._API_CLIENT_DETAILS
            % (
                self._client_id,
                self._include_doc_count,
                self._limit,
                self._offset,
                self._sort_by,
                self._sort_dir,
        ...
        if flag:
            if response.json() and "nodeList" in response.json():
                return (
                    response.json()["nodeList"][0] if len(response.json()["nodeList"]) > 0 else {}
                )
            raise SDKException("EdiscoveryClients", "106")
        self._response_not_success(response)
    ...
    def start_job(self, wait_for_job=False, wait_time=60, is_incr=True):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "GET",
            self._API_CRAWL % (self._client_id, self._data_source_id, self._type, self._operation),
        )
        if flag:
            if response.json():
            ...
            if flag:
                if response.json():
                    ...
                    if "errorCode" in response and response["errorCode"] != 0:
                        ...
                        "customMap" in response
                        and response["customMap"].get("name", "") == "statusObject"
                    ):
                        value_json = json.loads(response["customMap"]["value"])
                        if (
                            "response" in value_json
                            and isinstance(value_json["response"], dict)
                            and value_json["response"].get("status", "") == "finished"
                        ):
                            if not download:
                                return value_json["response"].get("downloadGuid", "")
                            else:
                                return self._do_stream_download(
                                    guid=value_json["response"].get("downloadGuid", ""),
    ...
    def wait_for_collection_job(self, wait_time=60):
        ...
        while True:
            ...
            if (
                int(status["state"]) == InventoryConstants.CRAWL_JOB_COMPLETE_STATE
            ):  # Finished State
                return
            elif (
                int(status["state"]) == InventoryConstants.CRAWL_JOB_COMPLETE_ERROR_STATE
            ):  # completed with error
                raise SDKException(
                    "EdiscoveryClients", "102", "Job status is marked as Completed with Error"
                )
            # STOPPING,STOPPED,ABORTING, ABORTED,EXCEPTION,UNKNOWN,SYNCING,PENDING
                ...
                raise SDKException(
                    "EdiscoveryClients", "102", "Job status is marked as Failed/Error/Pending"
                )
            else:
                time.sleep(10)
        ...
        if flag:
            if response.json() and "status" in response.json():
                ...
                if error_code != 0:
                    ...
                    raise SDKException("EdiscoveryClients", "102", error_message)
                raise SDKException(
                    "EdiscoveryClients", "102", "Something went wrong while fetching job history"
                )
            else:
        ...
        if flag:
            if response.json() and "status" in response.json():
                ...
                if error_code != 0:
                    ...
                    raise SDKException("EdiscoveryClients", "102", error_message)
                raise SDKException(
                    "EdiscoveryClients", "102", "Something went wrong while fetching job status"
                )
            else:
    ...
    def do_document_task(self, comment, doc_id=None, ds_id=None, consent=True, redact=False):
        ...
        if self._request_app == TargetApps.SDG.name:
            if not doc_id:
                raise SDKException(
                    "EdiscoveryClients", "102", "Document id is mandatory for request from SDG app"
                )
            if not ds_id:
                raise SDKException(
                    "EdiscoveryClients",
        ...
        req_json = {
            "nameValues": [
                {"name": f"ConsentFor_{self._request_review_set_id}_b", "value": f"{consent}"},
                {"name": f"CommentFor_{self._request_review_set_id}", "value": comment},
            ]
        }

        if self._request_app == TargetApps.SDG.name:
            req_json["nameValues"].append({"name": "q", "value": f"contentid:{doc_id}"})
            req_json["nameValues"].append({"name": "datasourceId", "value": f"{ds_id}"})
        ...
        if self._request_type == RequestConstants.RequestType.EXPORT.value:
            req_json["nameValues"].append(
                {"name": f"RedactMode_{self._request_review_set_id}_b", "value": f"{redact}"}
            )
        ...
        if flag:
            if response.json():
                ...
                if "jobId" in response.json():
                    ...
                    while retry > 0:
                        flag, response = self._cvpysdk_object.make_request(
                            "GET", self._API_TASK_AUDIT % self._client_id
                        )
                        if flag:
                            if response.json() and "auditDetails" in response.json():
                                ...
                                for record in audit_records:
                                    for message in record.get("messages", []):
                                        match = re.search(
                                            r"ApprovalWorkflowJobId: Set to \[(\d+)\]", message
                                        )
                                        if match:
                        ...
                        time.sleep(10)
                    raise SDKException(
                        "EdiscoveryClients", "102", "Workflow job fetch fails from Audit"
                    )
            raise SDKException("EdiscoveryClients", "102", "Workflow task failed")
        self._response_not_success(response)
    ...
    def configure_task(self, task_props):
        ...
        req_json = {
            "taskReq": {
                "tasks": [{"taskInfo": {"taskId": self._client_id}, "taskProps": task_props}]
            }
        }
        flag, response = self._cvpysdk_object.make_request(
        ...
        if flag:
            if response.json() and "msg" in response.json():
                ...
                return
            raise SDKException("EdiscoveryClients", "102", "Configure task failed")
        self._response_not_success(response)
...
class EdiscoveryDataSources:
    ...
    def __init__(self, commcell_object, class_object):
        ...
        from .file_storage_optimization import FsoServer, FsoServerGroups, FsoServers
                    ...
                    for collection in status["collections"]:
                        if "datasources" in collection:
                            ...
                            for data_source in data_sources:
                                ds_props = {
                                    EdiscoveryConstants.FIELD_DATA_SOURCE_DISPLAY_NAME: data_source[
                                        EdiscoveryConstants.FIELD_DISPLAY_NAME
                                    ],
                                    EdiscoveryConstants.FIELD_DATA_SOURCE_TYPE: data_source[
                                        EdiscoveryConstants.FIELD_DATA_SOURCE_TYPE
                                    ],
                                    EdiscoveryConstants.FIELD_DATA_SOURCE_ID: data_source[
                                        EdiscoveryConstants.FIELD_DATA_SOURCE_ID_NON_SEA
                                    ],
                                    EdiscoveryConstants.FIELD_DOCUMENT_COUNT: data_source.get(
                                        "status", {}
                                    ).get("totalcount", 0),
                                }
                                output[
                                    data_source[EdiscoveryConstants.FIELD_DISPLAY_NAME].lower()
                                ] = ds_props
                    return output
    ...
    def refresh(self):
        ...
        if self._app_source and self._app_source == TargetApps.SDG:
            self._ediscovery_client_props = (
                self._ediscovery_client_ops.get_ediscovery_project_details()
            )
    ...
    def add_fs_data_source(
        ...
        **kwargs,
        ...
        if (
            self._app_source_sub_type
            and self._app_source_sub_type == EdiscoveryConstants.FSO_SERVER_GROUPS
        ):
        ...
        if self._app_source.value not in plan_obj.content_indexing_props["targetApps"]:
            raise SDKException(
                "EdiscoveryClients", "102", "Plan is not marked with targetapp as FSO"
            )
        inv_obj = self._commcell_object.activate.inventory_manager().get(inventory_name)
        request_json = copy.deepcopy(EdiscoveryConstants.ADD_FS_REQ_JSON)
        request_json["datasourceId"] = inv_obj.inventory_id
        request_json["indexServerClientId"] = plan_obj.content_indexing_props[
        ...
        if not is_server_group:
            ...
            if not is_commvault_client:
                if "access_node" not in kwargs or "credential_name" not in kwargs:
                    raise SDKException(
                        "EdiscoveryClients", "102", "Access node/Credential information is missing"
                    )
                if not self._commcell_object.clients.has_client(kwargs.get("access_node")):
                    raise SDKException(
                        "EdiscoveryClients", "102", "Access node client is not present"
                    )
            inventory_resp = inv_obj.data_source.ds_handlers.get(
                EdiscoveryConstants.FS_SERVER_HANDLER_NAME
            ).get_handler_data(handler_filter=f"q=(name_idx:{server_name})&rows=1")
        ...
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "countryCode", "propertyValue": kwargs.get("country_code", "US")}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "co", "propertyValue": kwargs.get("country_name", "United States")}
        )
        request_json["datasources"][0]["properties"].append(
            {
                "propertyName": "contentid",
                "propertyValue": inventory_resp["contentid"]
                if not is_server_group
                else server_name,
            }
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "clientdisplayname", "propertyValue": server_name}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "dcplanid", "propertyValue": str(plan_obj.plan_id)}
        )
        request_json["datasources"][0]["properties"].append(
            {
                "propertyName": "name",
            ...
            request_json["datasources"][0]["properties"].append(
                {
                    ...
                    "propertyValue": str(
                        self._commcell_object.client_groups.get(server_name).clientgroup_id
                    ),
                }
            )
            request_json["datasources"][0]["properties"].append(
                {"propertyName": "ContentIndexingStatus", "propertyValue": str(0)}
            )
            request_json["datasources"][0]["properties"].append(
                {"propertyName": "BackedupStatus", "propertyValue": str(0)}
            )
        ...
        if source_type.value == EdiscoveryConstants.SourceType.BACKUP.value:
                ...
                request_json["datasources"][0]["properties"].append(
                    {
                        ...
                        "propertyValue": str(
                            EdiscoveryConstants.CrawlType.FILE_LEVEL_ANALYTICS.value
                        ),
                    }
                )
            else:
            ...
            if not is_commvault_client or "access_node" in kwargs:
                ...
                request_json["datasources"][0]["credentialId"] = (
                    self._commcell_object.credentials.get(credential_name).credential_id
                )
                request_json["datasources"][0]["properties"].append(
                    {"propertyName": "domainName", "propertyValue": inventory_resp["domainName"]}
                )
                request_json["datasources"][0]["properties"].append(
                    {"propertyName": "dNSHostName", "propertyValue": inventory_resp["dNSHostName"]}
                )
                request_json["datasources"][0]["accessNodes"][0]["clientId"] = int(
                    self._commcell_object.clients.get(kwargs.get("access_node")).client_id
                )
                request_json["datasources"][0]["accessNodes"][0]["clientName"] = kwargs.get(
                    "access_node"
                )
                        ...
                        if is_commvault_client:
                            ...
                            for client_name, _ in all_clients.items():
                                if client_name.lower().startswith(f"{data_source_name.lower()}_"):
                                    self._client_id = self._commcell_object.clients.get(
                                        client_name
                                    ).client_id
                                    break
                        self._ediscovery_client_ops = EdiscoveryClientOperations(
                            self._commcell_object, self
                        )
                    self.refresh()
    ...
    def add_o365_sdg_data_source(
        ...
        **kwargs,
        ...
        if self._app_source.value not in plan_obj.content_indexing_props["targetApps"]:
            raise SDKException(
                "EdiscoveryClients", "102", "Plan is not marked with targetapp as SDG"
            )
        o365_client = self._commcell_object.clients.get(server_name)
        backupset_id, subclient_id = self._get_o365_backupset_subclient_id(
            o365_client, datasource_type
        )
        request_json = copy.deepcopy(EdiscoveryConstants.ADD_O365_SDG_BACKED_UP_DS_REQ)
        request_json["clientId"] = self._client_id  # project source client id
        if (
            plan_obj.content_indexing_props.get("analyticsIndexServer", {}).get("clientId", None)
            is not None
        ):
        ...
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "clientdisplayname", "propertyValue": data_source_name}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "name", "propertyValue": data_source_name}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "dcplanid", "propertyValue": str(plan_obj.plan_id)}
        )
        ...
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "dNSHostName", "propertyValue": server_name}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "ClientId", "propertyValue": str(o365_client.client_id)}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "CAppBackupSetId", "propertyValue": str(backupset_id)}
        )
        request_json["datasources"][0]["properties"].append(
            {"propertyName": "backedupsubclientids", "propertyValue": str(subclient_id)}
        )
            ...
            if response.json() and "collections" in response.json():
                ...
                if "datasources" in collection:
                    ...
                    return EdiscoveryDatasource(
                        self._commcell_object,
                        data_source["datasourceId"],
                        EdiscoveryConstants.DATA_SOURCE_TYPES[datasource_type.value],
                        client_id=self._client_id,
                        app_type=self._app_source,
                    )
            if response.json() and "error" in response.json():
    ...
    def _get_o365_backupset_subclient_id(
        self, client, client_type=EdiscoveryConstants.ClientType.ONEDRIVE
    ):
        """
    ...
    def delete(self, data_source_name):
        ...
        flag, response = self._cvpysdk_object.make_request(
            "DELETE",
            self._API_DELETE % (self.get(data_source_name).data_source_id, self._client_id),
        )
        if flag:
    ...
    def has_data_source(self, data_source_name):
        ...
        if not isinstance(data_source_name, str):
            raise SDKException("EdiscoveryClients", "101")
        return (
            self._data_source_display_names
            and data_source_name.lower() in self._data_source_display_names
        )
...
class EdiscoveryDatasource:
    ...
    def __init__(
        self, commcell_object, data_source_id, data_source_type, client_id, app_type=TargetApps.FSO
    ):
    ...
    def _get_data_source_properties(self):
        ...
        """
        flag, response = self._cvpysdk_object.make_request(
            "GET", self._API_DATA_SOURCE % (self._data_source_id, self._data_source_type)
        )
        if flag:
            if response.json() and "collections" in response.json():
                collection = response.json()["collections"][0]
                ...
                if len(ds_list) == 1:
                    ...
                    self._crawl_type = self._get_property_value(
                        property_name=EdiscoveryConstants.FIELD_CRAWL_TYPE
                    )
                    self._dc_plan_id = self._get_property_value(
                        property_name=EdiscoveryConstants.FIELD_DC_PLAN_ID
                    )
    ...
    def tag_items(
        ...
        for tag in tags:
            ...
            if not tag_mgr.has_tag_set(tag_set_name=tag_split[0]):
                raise SDKException(
                    "EdiscoveryClients", "102", "Unable to find tagset in the commcell"
                )
        ...
        if not create_review:
            ...
            request_json["files"] = json.dumps(
                self._form_files_list(
                    ...
                    attr_list=EdiscoveryConstants.REVIEW_ACTION_IDA_SELECT_SET[
                        self.data_source_type_id
                    ],
                )
            )
            ...
            request_json["options"] = str(
                self._form_request_options(
                    ...
                    req_name=req_name
                    if req_name
                    else f"{self.data_source_name}_tag_{int(time.time())}",
                )
            )

        flag, response = self._cvpysdk_object.make_request("POST", api, request_json)
        if flag:
            if response.json():
                response = response.json()
                if "errorCode" in response and response["errorCode"] != 0:
    ...
    def get_job_history(self, limit=50, lookup_time=2160):
        ...
        """
        return self._jobs.finished_jobs(
            lookup_time=lookup_time, limit=limit, entity={"dataSourceId": self.data_source_id}
        )
    ...
    def wait_for_export(self, token, wait_time=60, download=True, download_location=os.getcwd()):
        ...
        """
        return self._ediscovery_client_ops.wait_for_export(
            token=token,
            wait_time=wait_time,
            download=download,
            download_location=download_location,
    ...
    def export(self, criteria=None, attr_list=None, params=None):
        ...
        return self._ediscovery_client_ops.export(
            criteria=criteria, attr_list=attr_list, params=params
        )
    ...
    def _form_request_options(self, req_name, reviewers, approvers, document_ids=None):
        ...
        if document_ids:
            ...
            options["ReviewCriteria"] = json.dumps({"Files": file_names})
        else:
        ...
        for user in reviewers:
            if not self._commcell_object.users.has_user(user):
                raise SDKException(
                    "EdiscoveryClients", "102", f"Unable to find reviewer user : {user}"
                )
            user_obj = self._commcell_object.users.get(user)
            reviewers_list.append({"id": user_obj.user_id, "name": user_obj.user_name})
        ...
        for user in approvers:
            if not self._commcell_object.users.has_user(user):
                raise SDKException(
                    "EdiscoveryClients", "102", f"Unable to find approver user : {user}"
                )
            user_obj = self._commcell_object.users.get(user)
            approvers_list.append({"id": user_obj.user_id, "name": user_obj.user_name})
    ...
    def review_action(
        ...
        **kwargs,
        ...
        if not isinstance(action_type, EdiscoveryConstants.ReviewActions):
            raise SDKException("EdiscoveryClients", "101")
        if (
            self._app_type.value == TargetApps.FSO.value
            and action_type.value not in EdiscoveryConstants.REVIEW_ACTION_FSO_SUPPORTED
        ):
            raise SDKException(
                "EdiscoveryClients", "102", f"{action_type.value} is not supported for FSO app"
            )
        if (
            self._app_type.value == TargetApps.SDG.value
            and action_type.value not in EdiscoveryConstants.REVIEW_ACTION_SDG_SUPPORTED
        ):
            raise SDKException(
                "EdiscoveryClients", "102", f"{action_type.value} is not supported for SDG app"
            )
        attr_list = None
        api = self._API_ACTIONS
        create_review = kwargs.get("create_review", False)
        if (
            action_type == EdiscoveryConstants.ReviewActions.DELETE
            or action_type == EdiscoveryConstants.ReviewActions.MOVE
        ):
        ...
        if create_review:
            if not reviewers or not approvers:
                raise SDKException(
                    "EdiscoveryClients", "102", "Reviewers/Approvers missing in input"
                )
        if self.data_source_type_id not in EdiscoveryConstants.REVIEW_ACTION_IDA_SELECT_SET:
            raise SDKException(
                "EdiscoveryClients", "102", "Not supported data source for review action"
            )
        ...
        if action_type.value == EdiscoveryConstants.ReviewActions.DELETE.value:
            ...
            if (
                "destination" not in kwargs
                or "user_name" not in kwargs
                or "password" not in kwargs
            ):
                raise SDKException(
                    "EdiscoveryClients", "102", "Required params missing for move operation"
                )
            ...
            if "retain_month" not in kwargs:
                raise SDKException(
                    "EdiscoveryClients", "102", "Retention month input missing for this operation"
                )
            if not create_review:
            ...
            if not ignore_all and "ignore_risk_type" not in kwargs:
                raise SDKException(
                    "EdiscoveryClients",
                    "102",
                    "Ignore risk type details missing for this operation",
                )
            if not create_review:
                if ignore_all:
                    ...
                    request_json["ignoreRisksReq"]["ignoreRiskTypeList"] = kwargs.get(
                        "ignore_risk_type"
                    )
                request_json["remActionRequest"]["dataSourceId"] = self._data_source_id
            else:
        ...
        if document_ids:
            ...
            if not create_review:
                ...
                request_json["remActionRequest"]["searchRequest"] = json.dumps(
                    self._ediscovery_client_ops.form_search_params(
                        ...
                        params={"start": "0"},
                        query=query,
                        is_separate_attr=True,
                    )
                )

            else:
                request_json["files"] = json.dumps(
                    self._form_files_list(document_ids=document_ids, attr_list=attr_list)
                )
        else:
            ...
            if not create_review:
                request_json["remActionRequest"]["searchRequest"] = (
                    EdiscoveryConstants.REVIEW_ACTION_BULK_SEARCH_REQ
                )
                request_json["remActionRequest"]["isBulkOperation"] = True
                request_json["remActionRequest"]["handlerId"] = (
                    self._ediscovery_client_ops.get_handler_id()
                )
            else:
        ...
        if create_review:
            ...
            request_json["options"] = json.dumps(
                self._form_request_options(
                    ...
                    req_name=req_name
                    if req_name
                    else f"{self.data_source_name}_{action_type.name}_{int(time.time())}",
                )
            )
    ...
    def data_source_type_id(self):
        ...
        """
        position = list(EdiscoveryConstants.DATA_SOURCE_TYPES.values()).index(
