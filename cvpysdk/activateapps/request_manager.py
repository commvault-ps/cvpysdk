...
class Requests:
    ...
    def add(self, req_name, req_type, requestor, criteria, **kwargs):
        ...
        if (
            ...
            or not isinstance(req_name, str)
            or not isinstance(req_type, RequestConstants.RequestType)
            or not isinstance(requestor, str)
        ):
            raise SDKException("RequestManager", "101")
        entities = []
        for key, value in criteria.items():
            entities.append({"name": key, "values": value})
        req_json = {
            "name": req_name,
            "type": req_type.name,
        ...
        flag, response = self._cvpysdk_object.make_request(
            "POST", self._API_GET_REQUESTS, req_json
        )
        if flag:
            if response.json():
...
class Request:
    ...
    def __init__(self, commcell_object, req_name, req_id=None):
        ...
        if req_id:
            ...
            self._req_id = (
                self._commcell_object.activate.request_manager().get(req_name).request_id
            )
        self.refresh()
    ...
    def _get_request_properties(self):
        ...
        """
        flag, response = self._cvpysdk_object.make_request(
            "GET", self._API_REQ_DETAILS % self._req_id
        )
        if flag:
            if response.json():
                ...
                self._req_criteria = self._get_property_value(
                    req_properties=response.json().get("properties", []),
                    prop_name=RequestConstants.PROPERTY_REVIEW_CRIERIA,
                )
                if not self._req_criteria:
                    # SDG request based on entities
                    self._req_criteria = self._get_property_value(
                        req_properties=response.json().get("properties", []),
                        prop_name=RequestConstants.PROPERTY_ENTITIES,
                    )
    ...
    def _get_valid_projects(self):
        ...
        output = {}
        flag, response = self._cvpysdk_object.make_request(
            "GET", self._API_VALID_PROJECTS % self._req_id
        )
        if flag:
    ...
    def configure(self, projects, reviewers, approvers):
        ...
        if self.request_app != TargetApps.SDG.name:
            raise SDKException(
                "RequestManager",
                "102",
                "Configuring Request is supported only for SDG App request",
            )
        if (
        ...
        for project in projects:
            if project.lower() not in valid_projects:
                raise SDKException(
                    "RequestManager",
                    "102",
                    f"Not a valid project {project} to associate to request",
                )
        ...
        for user in reviewers:
            if not self._commcell_object.users.has_user(user):
                raise SDKException(
                    "RequestManager", "102", f"Unable to find reviewer user : {user}"
                )
        ...
        for user in approvers:
            if not self._commcell_object.users.has_user(user):
                raise SDKException(
                    "RequestManager", "102", f"Unable to find approver user : {user}"
                )
            user_obj = self._commcell_object.users.get(user)
        ...
        flag, response = self._cvpysdk_object.make_request(
            "PUT", self._API_REQ_CONFIGURE % self._req_id, req_json
        )
        if flag:
            if response.json():
                if "errorCode" in response.json() and response.json()["errorCode"] != 0:
    ...
    def get_document_details(self, criteria=None, attr_list=None, query="*:*", start=0, rows=10):
        ...
        payload = self._ediscovery_client_ops.form_search_params(
            ...
            criteria=criteria,
            attr_list=attr_list,
            params={"start": f"{start}", "rows": f"{rows}"},
        )
        flag, response = self._cvpysdk_object.make_request("POST", api, payload)
        if flag:
            if response.json():
                if "response" in response.json():
                    return response.json()["response"]["numFound"], response.json()["response"][
                        "docs"
                    ]
            raise SDKException(
                "RequestManager", "102", "Failed to get document details for this request"
            )
        self._response_not_success(response)
    ...
    def review_stats(self):
        ...
        if flag:
            if response.json():
                if "response" in response.json():
                    output[RequestConstants.FIELD_DOC_COUNT] = response.json()["response"][
                        "numFound"
                    ]
                if "facets" in response.json():
                    facets = response.json()["facets"]
                    output[RequestConstants.FIELD_REVIEWED] = facets[
                        RequestConstants.FACET_REVIEWED % review_set_id
                    ][RequestConstants.FACET_COUNT]
                    output[RequestConstants.FIELD_NOT_REVIEWED] = facets[
                        RequestConstants.FACET_NOT_REVIEWED % review_set_id
                    ][RequestConstants.FACET_COUNT]
                    output[RequestConstants.FIELD_ACCEPTED] = facets[
                        RequestConstants.FACET_ACCEPTED % review_set_id
                    ][RequestConstants.FACET_COUNT]
                    output[RequestConstants.FIELD_DECLINED] = facets[
                        RequestConstants.FACET_DECLINED % review_set_id
                    ][RequestConstants.FACET_COUNT]
                    if self.request_type == RequestConstants.RequestType.EXPORT.value:
                        output[RequestConstants.FIELD_REDACTED] = facets[
                            RequestConstants.FACET_REDACTED % review_set_id
                        ][RequestConstants.FACET_COUNT]
                        output[RequestConstants.FIELD_NOT_REDACTED] = facets[
                            RequestConstants.FACET_NOT_REDACTED % review_set_id
                        ][RequestConstants.FACET_COUNT]
                return output
            raise SDKException(
                "RequestManager", "102", "Failed to get review stats for this request"
            )
        self._response_not_success(response)
    ...
    def mark_review_complete(self):
        ...
        if int(stats[RequestConstants.FIELD_DOC_COUNT]) != int(
            stats[RequestConstants.FIELD_REVIEWED]
        ):
            raise SDKException("RequestManager", "109")
        task_prop = [{"attrVal": "ReviewCompleted", "attrName": "progress"}]
    ...
    def review_document(self, comment, doc_id=None, ds_id=None, consent=True, redact=False):
        ...
        Raises:
            ...
            SDKException:
                ...
                if input is not valid
        """
        return self._ediscovery_client_ops.do_document_task(
            doc_id=doc_id, comment=comment, consent=consent, redact=redact, ds_id=ds_id
        )
    ...
    def give_approval(self, workflow_job_id, action="Approve"):
        ...
        self._commcell_object.workflows.submit_interaction(
            interaction=interaction_props, input_xml="", action=action
        )
