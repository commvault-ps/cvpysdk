...
class FsoServers:
    ...
    def get(self, server_name):
        ...
        return FsoServer(
            commcell_object=self._commcell_object, server_name=server_name, server_id=server_id
        )
...
class FsoServer:
    ...
    def __init__(self, commcell_object, server_name, server_id=None):
        ...
        if server_id:
            ...
            self._server_id = (
                self._commcell_object.activate.file_storage_optimization()
                .get(server_name)
                .server_id
            )
    ...
    def start_collection(self):
        ...
        request_json["taskInfo"]["task"]["taskName"] = (
            f"Cvpysdk_FSO_server_Crawl_{self._server_name}_{int(time.time())}"
        )
    ...
    def delete_schedule(self):
        ...
        if not self._schedule_obj:
            raise SDKException(
                "FileStorageOptimization", "102", "No schedule is associated to this FSO Server"
            )
...
class FsoServerGroup:
    ...
    def __init__(self, commcell_object, server_group_name, server_id=None):
        ...
        if server_id:
            ...
            self._server_id = (
                self._commcell_object.activate.file_storage_optimization(FsoTypes.SERVER_GROUPS)
    ...
    def start_collection(self):
        ...
        request_json["taskInfo"]["associations"][0]["_type_"] = (
            28  # server group level job collection
        )
        request_json["taskInfo"]["task"]["taskName"] = (
            f"Cvpysdk_FSO_server_Crawl_{self._server_group_name}_{int(time.time())}"
        )
        flag, response = self._commcell_object._cvpysdk_object.make_request(
            "POST", self._CREATE_POLICY, request_json
        )
