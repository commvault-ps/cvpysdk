...
class SNMPConfigurations:
    ...
    def __init__(self, commcell_object: object) -> None:
        ...
        self.privacy_algo_ids = {"None": 0, "CBC_DES": 1, "CFB128_AES128": 2, "CBC_AES128": 3}
    ...
    def _get_all_configs(self) -> dict:
        ...
        #ai-gen-doc
        """
        flag, response = self._cvpysdk_object.make_request("GET", self._SNMP_CONFIG)
        if flag:
            if response and "snmv3InfoList" in response.json():
                return {
                    config["hostName"]: {
                        "privacy_algorithm": self.privacy_algo_names[config["privacyAlgorithm"]],
                        "authentication_algorithm": self.encrypt_algo_names[
    ...
    def _update_config(
        self, optype: int, config_hostname: str, username: str, password: str = "", **kwargs: Any
    ) -> None:
        ...
        payload = {
            ...
            "snmv3Info": {
                ...
                "encryptAlgorithm": self.encrypt_algo_ids[
                    kwargs.get("authentication_algorithm", "HMAC_MD5")
                ],
                "privacyAlgorithm": 0,
        ...
        if password:
            payload["snmv3Info"]["userAccount"]["password"] = base64.b64encode(
                password.encode()
            ).decode()
        if (priv_algo := kwargs.get("privacy_algorithm")) not in ["None", None]:
            ...
            if priv_pass := kwargs.get("privacy_password"):
                payload["snmv3Info"]["privacyPassword"] = base64.b64encode(
                    priv_pass.encode()
                ).decode()
        flag, response = self._cvpysdk_object.make_request("POST", self._SNMP_CONFIG, payload)
        if flag:
            if response.json():
                self.refresh()
                return
                ...
                raise SDKException(
                    "Commcell", "101", f"Failed to update SNMP configuration: {config_hostname}"
                )
        response_string = self._update_response_(response.content)
        raise SDKException("Response", "101", response_string)
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        if config_hostname not in self.all_configs:
            raise SDKException(
                "Commcell", "102", f"Configuration {config_hostname} does not exist"
            )
        ...
        self._update_config(
            ...
            password,
            id=old["id"],
            authentication_algorithm=kwargs.get(
                "authentication_algorithm", old["authentication_algorithm"]
            ),
